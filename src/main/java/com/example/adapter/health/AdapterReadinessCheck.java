...
public class AdapterReadinessCheck implements HealthCheck {
    ...
    public HealthCheckResponse call() {
        if (registry.loaded()) {
            return HealthCheckResponse.named("adapter-readiness").up()
        ...
        MappingLoadFailure f = registry.failure();
        if (f != null) {
            return HealthCheckResponse.named("adapter-readiness").down()
                    .withData("code", f.code())
                    .withData("reason", f.message())
                    .withData("ref", f.ref())
                    .build();
        }
