...
public class ExcelWorkbookLoader {
    ...
    private void requireSheet(Sheet s, String n) { if (s == null) throw new ExcelSheetNotFoundException(n); }
    ...
    private String cell(Row row, Map<String,Integer> h, String col) {
        Integer idx = h.get(col); if (idx == null) return null;
        Cell c = row.getCell(idx, Row.MissingCellPolicy.RETURN_BLANK_AS_NULL); if (c == null) return null;
        return switch (c.getCellType()) {
            case STRING -> c.getStringCellValue().trim();
            case NUMERIC -> { double n = c.getNumericCellValue(); yield Math.floor(n) == n ? String.valueOf((long)n) : String.valueOf(n); }
    ...
    private String req(Row row, Map<String,Integer> h, String sheet, String col) {
        Integer idx = h.get(col); if (idx == null) throw new ExcelMissingColumnException(sheet, col);
        String v = cell(row, h, col); if (v == null || v.isBlank()) throw new ExcelMissingCellValueException(sheet, row.getRowNum(), idx, col);
        return v;
    }
    ...
    private int reqInt(Row row, Map<String,Integer> h, String sheet, String col) {
        ...
        catch (NumberFormatException e) { throw new ExcelInvalidCellFormatException(sheet, row.getRowNum(), idx, col, "integer", v, e); }
    }
