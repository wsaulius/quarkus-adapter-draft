...
public class AdapterService {
    ...
    public ExecutionResult execute(String tenant, String environment, String path, String method, JsonNode body) {
        ExecutionContext result = pipeline.execute(ExecutionContext.initial(
                new RouteKey(tenant, environment, method), path, body));

        return new ExecutionResult(result.route().route().targetSystem(), result.route().route().planId(),
                result.executionMode(), result.pathParams(), result.stepResults(), result.finalResponse());
    }
}
