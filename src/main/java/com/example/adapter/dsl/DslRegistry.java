...
public class DslRegistry {
    ...
    public void replaceAll(Map<String, CompiledTransform> transforms) { transformsRef.set(Map.copyOf(transforms)); }
    public CompiledTransform transform(String name) {
        CompiledTransform t = transformsRef.get().get(name);
        if (t == null) throw new IllegalArgumentException("Unknown transform_ref: " + name);
        return t;
    }
    public Map<String, CompiledTransform> allTransforms() { return transformsRef.get(); }
}
